...
public class Game implements Strategy {
	...
	for (int i = 0; i < (agents.length - 1); i++) {
	    ...
	    // agents[i].agentAct(historyChoise[i]);
	    agents[i].agentAct(historyChoiseStatic,i);//根据历史来决定买和卖，也就是action的值，为-1
	    // 或者1
	...
	for (int i = 0; i < (agents.length); i++) {
	    // agent[i].feedback(historyChoise[i],caculateThisTurnPrice(
	    // currentChoise),
	    // i);
	...
	// System.out.println("history "+i+"is :"+historyChoise[i]);
    }
